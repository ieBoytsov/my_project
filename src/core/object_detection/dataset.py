...
class KITTI2D(Dataset):
    ...
    def _read_image(self, index):
        ...
        if self.train:
            ...
            return torch.from_numpy(
                self._pad_resize_image(augmented_image, self.image_size)
            )

        return torch.from_numpy(
            self._pad_resize_image(np.asarray(image), self.image_size)
        )
    ...
    def _read_label(self, index):
        ...
        if os.path.exists(label_filename):
            ...
            w, h, pad, padded_h, padded_w = (
                self.state_variables["w"],
                self.state_variables["h"],
                self.state_variables["pad"],
                self.state_variables["padded_h"],
                self.state_variables["padded_w"],
            )
    ...
    def _load_filenames(self):
        ...
        self.image_filenames = self.image_filenames[
            : int(len(self.image_filenames) * self.fraction)
        ]

        # Create splitted dataset
        if self.train:
            self.image_filenames = self.image_filenames[
                : int(len(self.image_filenames) * self.split_ratio)
            ]
        else:
            self.image_filenames = self.image_filenames[
                int(len(self.image_filenames) * self.split_ratio) :
